...
const ActionsModal = ({ showModal, handleOnClose, 
  isWalletConnected, 
  userBalance,minimumBalanceToPlay,tokenName,
  ...
  modalContent=
  <>
    You are ready to start challenging other players. Click the button below to sign.<br/>
    You will be charged <b>{minimumBalanceToPlay} {tokenName}</b>.
  </>
  modalAction=
  <button disabled={!enableSigner}
  onClick={signMatchRequest}
    className='btn'>Post Challenge</button>
  ...
  if(userMatchRequestStatus=="reject"){
    ...
    modalContent=
    <>
      You must sign a match request. Click the button below to sign.<br/>
    </>
    modalAction=
    <button disabled={!enableSigner} 
    onClick={signMatchRequest}
  ...
  if(userMatchRequestStatus=="timeout"){
    ...
    modalContent=
    <>
      Your last challenge has ended. Please try again.<br/>
    </>
    modalAction=<button onClick={signMatchRequest} className='btn'>Post Challenge</button>
  }
  if(userMatchRequestStatus=="accept"){
    ...
    modalAction=<button onClick={()=>{handleOnClose()}}className='btn'>Ok, I Understand</button>
  }
  ...
  if (!userHasEnoughBalance && userBalance) {
    const needToPay=minimumBalanceToPlay-userBalance;
      ...
      start playing games.<br/>Your currrent balance is <b>{userBalance} {tokenName}</b>.
      You need {needToPay} {tokenName} more.
      </>
      modalIcon=<GiPiggyBank/>
      modalAction=<button onClick={()=>{topUpWallet()}}className='btn'>Add {needToPay} {tokenName}</button>
  }
